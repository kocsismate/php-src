...
foreach ($it as $file) {
    ...
    if ($lang === 'c') {
        ...
        foreach ($psr12Includes as $include) {
            if (strpos($path, $include) !== false) {
        ...
        $code = transformTestCode($code, function (string $code) use ($path, $psr12Includes) {
            $code = stripTrailingWhitespace($code);
            $code = reindentToSpaces($code);

            foreach ($psr12Includes as $include) {
                if (strpos($path, $include) !== false) {
                    return applyPsr12($code) . "\n?>\n";
                }
            }
