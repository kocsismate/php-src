...
static ZEND_COLD zend_never_inline void zend_initonly_property_assignment_error(zend_class_entry *ce, zend_string *property_name) /* {{{ */
{
	zend_throw_error(NULL, "Cannot modify initonly property %s::$%s after initialization", ZSTR_VAL(ce->name), ZSTR_VAL(property_name));
}
/* }}} */

static ZEND_COLD zend_never_inline void zend_initonly_property_unset_error(zend_class_entry *ce, zend_string *property_name) /* {{{ */
{
	zend_throw_error(NULL, "Cannot unset initonly property %s::$%s", ZSTR_VAL(ce->name), ZSTR_VAL(property_name));
}
/* }}} */
...
ZEND_API zval *zend_std_write_property(zend_object *zobj, zend_string *name, zval *value, void **cache_slot) /* {{{ */
{
	...
	if (EXPECTED(IS_VALID_PROPERTY_OFFSET(property_offset))) {
		...
		if (Z_TYPE_P(variable_ptr) != IS_UNDEF) {
			if (UNEXPECTED(prop_info && prop_info->flags & ZEND_ACC_INITONLY && Z_PROP_FLAG_P(variable_ptr) != IS_PROP_UNINIT)) {
				zend_initonly_property_assignment_error(zobj->ce, name);
	...
	if (EXPECTED(IS_VALID_PROPERTY_OFFSET(property_offset))) {
		...
		if (Z_TYPE_P(slot) != IS_UNDEF) {
			if (UNEXPECTED(prop_info && prop_info->flags & ZEND_ACC_FINAL)) {
				zend_initonly_property_unset_error(zobj->ce, name);
				return;
			}
...
undeclared_property:
	...
	if (UNEXPECTED((type == BP_VAR_RW || type == BP_VAR_W) && property_info->flags & ZEND_ACC_INITONLY && Z_TYPE_P(ret) != IS_UNDEF)) {
		zend_throw_error(NULL, "Cannot modify initonly static property %s::$%s after initialization",
